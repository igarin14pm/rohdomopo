import { beforeEach, expect, test } from './modules/test.js';
...
await test('`RohdomopoTimer.isCountingDown`', () => {
...
await test('`setTimeout()` - 1', async () => {
  const timeout1Started = new Date();

  await TimersPromises.setTimeout(100);
  const timeout1Finished = new Date();
  
  expect(timeout1Finished.getTime()).toBeGreaterThan(timeout1Started.setMilliseconds(timeout1Started.getMilliseconds() + 90));
  expect(timeout1Finished.getTime()).toBeLessThan(timeout1Started.setMilliseconds(timeout1Started.getMilliseconds() + 20));
});

await test('`setTimeout()` - 2', async () => {
  const timeout2Started = new Date();

  await TimersPromises.setTimeout(250);
  const timeout2Finished = new Date();
  
  expect(timeout2Finished.getTime()).toBeGreaterThan(timeout2Started.setMilliseconds(timeout2Started.getMilliseconds() + 240));
  expect(timeout2Finished.getTime()).toBeLessThan(timeout2Started.setMilliseconds(timeout2Started.getMilliseconds() + 20));
})

await test('`setInterval()` - 1', async () => {
  ...
  expect(count).toBe(0);
  expect(intervalFinished.getTime()).toBeGreaterThan(intervalStarted.setMilliseconds(intervalStarted.getMilliseconds() + 490));
  expect(intervalFinished.getTime()).toBeLessThan(intervalStarted.setMilliseconds(intervalStarted.getMilliseconds() + 20));
});

await test('`setInterval()` - 2', async () => {
  const intervalStarted = new Date();
  ...
  expect(count).toBe(2);
  expect(intervalFinished.getTime()).toBeGreaterThan(intervalStarted.setMilliseconds(intervalStarted.getMilliseconds() + 290));
  expect(intervalFinished.getTime()).toBeLessThan(intervalStarted.setMilliseconds(intervalStarted.getMilliseconds() + 20));
});
